...
query_gitstatusd() {
    ...
    # Hello handshake
    printf '}\x1f\x1e' >&"${GITD[1]}" 2>/dev/null || { kill "$GITD_PID" 2>/dev/null; wait "$GITD_PID" 2>/dev/null; return 1; }
    local hello
    IFS= read -r -d $'\x1e' -t 2 hello <&"${GITD[0]}" 2>/dev/null || { kill "$GITD_PID" 2>/dev/null; wait "$GITD_PID" 2>/dev/null; return 1; }

    # Query
    printf '1 \x1f%s\x1f0\x1e' "$cwd" >&"${GITD[1]}" 2>/dev/null || { kill "$GITD_PID" 2>/dev/null; wait "$GITD_PID" 2>/dev/null; return 1; }
    local resp
    IFS= read -r -d $'\x1e' -t 2 resp <&"${GITD[0]}" 2>/dev/null || { kill "$GITD_PID" 2>/dev/null; wait "$GITD_PID" 2>/dev/null; return 1; }
